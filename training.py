...
def main():
    ...
    bert_optimizer = optim.Adam(list(bert_model.parameters()), lr=2e-5,)
    ...
    dst = dataloader.QuestionReader(root="/sdd1/amit/DAV/project/dataset/split",)
    batch_size = 8
    
    for epoch in range(start_epoch, end_epoch):
        start_time = time.time()
        train_loader = data.DataLoader(dst, batch_size=batch_size, shuffle=True, drop_last=True,)
        
        #train_loader = cycle(data.DataLoader(dst, batch_size=batch_size, shuffle=True, drop_last=True,))
        #for iteration in range(int(len(dst) / batch_size)):
        for iteration, batch in enumerate(train_loader):
            ...
            if((iteration+1)%5000 == 0):
                iteration_buffer = 40000
                accuracy = computeValidationAccuracy(bert_model, tokenizer, max_length=512)
                print("iteration:", iteration+iteration_buffer)
                print("validation accuracy:", accuracy)
                print("5000 iteration time:", time.time()-start_time)
                print("===================================================================\n\n")
                f.write("epoch, iteration : ")
                f.write(str(epoch))
                f.write(" : ")
                f.write(str(iteration+iteration_buffer))
                f.write("\n")
                f.write("validation accuracy : ")
                f.write(str(accuracy))
                f.write("\n")
                f.write("5000 iteration time : ")
                f.write(str(time.time()-start_time))
                f.write("\n=================================================================\n\n")
                torch.save(bert_model.state_dict(), os.path.join('checkpoints', 'bert_model'))
                bert_model.train()
                start_time = time.time()
    f.close()
